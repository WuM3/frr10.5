...
struct bgp_linkstate_tlv_info bgp_linkstate_tlv_infos[BGP_LS_TLV_MAX + 1] = {
	...
	[BGP_LS_SPF_TLV_Sequence_Number] = {"SPF Sequence Number", 8, 8, UNDEF_MULTPL},
	[BGP_LS_SPF_TLV_SPF_Status] = {"SPF Status", 1, 1, UNDEF_MULTPL},
	[BGP_LS_SPF_TLV_AF_Link_Descriptor] = {"AF Link Descriptor", 1, 1, UNDEF_MULTPL}
};
/* clang-format on */
...
/*
 * RFC9815 BGPLS-SPF implementation structures and helpers.
 * These implement the SPF-specific LSDB with sequence-number based updates.
 */

/* SPF LSDB Node record (RFC9815) */
struct bgp_ls_spf_node {
	uint64_t key;             /* Hash key from node descriptors */
	uint64_t seq_num;         /* TLV 1181: Sequence Number (mandatory) */
	uint8_t spf_status;       /* TLV 1184: SPF Status (0=reachable) */
	uint32_t igp_router_id;   /* For SPF calculation */
	bool valid;               /* Entry is active */
};

/* SPF LSDB Link record (RFC9815) */
struct bgp_ls_spf_link {
	uint64_t key;             /* Hash key from link descriptors */
	uint64_t seq_num;         /* TLV 1181: Sequence Number (mandatory) */
	uint8_t spf_status;       /* TLV 1184: SPF Status (0=reachable) */
	uint32_t igp_metric;      /* TLV 1095: IGP Metric (mandatory for link) */
	uint32_t local_node;      /* Link endpoints for graph */
	uint32_t remote_node;
	bool valid;
};

/* SPF LSDB Prefix record (RFC9815) */
struct bgp_ls_spf_prefix {
	uint64_t key;             /* Hash key from prefix descriptors */
	uint64_t seq_num;         /* TLV 1181: Sequence Number (mandatory) */
	uint8_t spf_status;       /* TLV 1184: SPF Status (0=reachable) */
	uint32_t prefix_metric;   /* TLV 1155: Prefix Metric (mandatory) */
	uint32_t node_id;         /* Originating node for SPF */
	bool valid;
};

/* BGPLS-SPF context: LSDB + graph + dirty flags */
struct bgp_ls_spf_ctx {
	struct peer *peer;
	/* LSDB tables (simplified: in production use hash tables) */
	struct bgp_ls_spf_node nodes[256];
	struct bgp_ls_spf_link links[512];
	struct bgp_ls_spf_prefix prefixes[256];
	uint16_t node_count;
	uint16_t link_count;
	uint16_t prefix_count;
	/* Topology change tracking */
	bool topology_dirty;      /* LSDB changed, need graph update */
	bool spf_needed;          /* Graph changed, need SPF recompute */
};

/*
 * Get or create SPF context for peer.
 * Flow:
 *   1. Lookup per-BGP-instance context (future: per-VRF)
 *   2. Initialize on first use
 *   3. Return context pointer
 */
static struct bgp_ls_spf_ctx *bgp_ls_spf_get_ctx(struct peer *peer)
{
	/* Static context for now; production should use peer->bgp->spf_ctx */
	static struct bgp_ls_spf_ctx ctx;
...
/*
 * Update SPF LSDB from a single NLRI + Attribute.
 * RFC9815 Flow:
 *   1. Parse BGP-LS Attribute TLVs (attr->link_state):
 *      - TLV 1181 (Sequence Number): mandatory for version control
 ...
 *      - If new seq > old seq: replace entry
 *      - If new seq < old seq: ignore (stale)
 *      - If new seq == old seq: update in-place
 *   4. Withdraw handling:
 *      - If withdraw==true: mark entry invalid (implicit withdraw)
 *      - No sequence check on withdraw
 *   5. Set topology_dirty flag if LSDB changed
 */
static void bgp_ls_spf_lsdb_update(struct bgp_ls_spf_ctx *ctx,
			       struct attr *attr, const struct prefix *p,
			       bool withdraw)
{
	uint64_t seq_num = 0;
	uint8_t spf_status = 0;  /* Default: reachable (RFC9815 Section 5.2.1) */
	uint32_t metric = 0;
	uint8_t *attr_pnt, *attr_lim;
	uint16_t nlri_type;
	uint64_t key;
	bool found_seq = false;

	/* Step 1: Parse BGP-LS Attribute TLVs (mandatory: seq_num) */
	if (attr && attr->link_state && !withdraw) {
		attr_pnt = attr->link_state->data;
		attr_lim = attr_pnt + attr->link_state->length;

		while (attr_pnt + 4 <= attr_lim) {
			uint16_t tlv_type = pnt_decode16(&attr_pnt);
			uint16_t tlv_len = pnt_decode16(&attr_pnt);
			
			if (attr_pnt + tlv_len > attr_lim)
				break;

			switch (tlv_type) {
			case BGP_LS_SPF_TLV_Sequence_Number:  /* 1181 */
				if (tlv_len == 8) {
					seq_num = pnt_decode64(&attr_pnt);
					found_seq = true;
				}
				break;
			case BGP_LS_SPF_TLV_SPF_Status:       /* 1184 */
				if (tlv_len == 1)
					spf_status = pnt_decode8(&attr_pnt);
				break;
			case BGP_LS_TLV_IGP_METRIC:           /* 1095 */
				if (tlv_len <= 3)
					metric = (tlv_len == 3) ? pnt_decode24(&attr_pnt)
								: (tlv_len == 2) ? pnt_decode16(&attr_pnt)
								: pnt_decode8(&attr_pnt);
				break;
			case BGP_LS_TLV_PREFIX_METRIC:        /* 1155 */
				if (tlv_len <= 4)
					metric = (tlv_len == 4) ? pnt_decode32(&attr_pnt)
								: pnt_decode24(&attr_pnt);
				break;
			default:
				attr_pnt += tlv_len;  /* Skip unknown TLVs */
				break;
			}
		}

		/* RFC9815: Sequence Number is mandatory for UPDATE */
		if (!found_seq) {
			flog_warn(EC_BGP_LINKSTATE_PACKET,
				  "BGPLS-SPF: missing mandatory Sequence Number TLV");
			return;
		}
	}
	...
	switch (nlri_type) {
	case 1:  /* Node NLRI (RFC7752) */
		for (int i = 0; i < ctx->node_count; i++) {
			if (ctx->nodes[i].key == key) {
				if (withdraw) {
					ctx->nodes[i].valid = false;
					ctx->topology_dirty = true;
					return;
				}
				/* Sequence-number check */
				if (seq_num < ctx->nodes[i].seq_num)
					return;  /* Stale update, ignore */
				ctx->nodes[i].seq_num = seq_num;
				ctx->nodes[i].spf_status = spf_status;
				ctx->topology_dirty = true;
				return;
			}
		}
		/* New node entry */
		if (!withdraw && ctx->node_count < 256) {
			ctx->nodes[ctx->node_count].key = key;
			ctx->nodes[ctx->node_count].seq_num = seq_num;
			ctx->nodes[ctx->node_count].spf_status = spf_status;
			ctx->nodes[ctx->node_count].valid = true;
			ctx->node_count++;
			ctx->topology_dirty = true;
		}
		break;

	case 2:  /* Link NLRI (RFC7752) */
		for (int i = 0; i < ctx->link_count; i++) {
			if (ctx->links[i].key == key) {
				if (withdraw) {
					ctx->links[i].valid = false;
					ctx->topology_dirty = true;
					return;
				}
				if (seq_num < ctx->links[i].seq_num)
					return;
				ctx->links[i].seq_num = seq_num;
				ctx->links[i].spf_status = spf_status;
				ctx->links[i].igp_metric = metric;
				ctx->topology_dirty = true;
				return;
			}
		}
		if (!withdraw && ctx->link_count < 512) {
			ctx->links[ctx->link_count].key = key;
			ctx->links[ctx->link_count].seq_num = seq_num;
			ctx->links[ctx->link_count].spf_status = spf_status;
			ctx->links[ctx->link_count].igp_metric = metric;
			ctx->links[ctx->link_count].valid = true;
			ctx->link_count++;
			ctx->topology_dirty = true;
		}
		break;

	case 3:  /* IPv4 Prefix NLRI */
	case 4:  /* IPv6 Prefix NLRI */
		for (int i = 0; i < ctx->prefix_count; i++) {
			if (ctx->prefixes[i].key == key) {
				if (withdraw) {
					ctx->prefixes[i].valid = false;
					ctx->topology_dirty = true;
					return;
				}
				if (seq_num < ctx->prefixes[i].seq_num)
					return;
				ctx->prefixes[i].seq_num = seq_num;
				ctx->prefixes[i].spf_status = spf_status;
				ctx->prefixes[i].prefix_metric = metric;
				ctx->topology_dirty = true;
				return;
			}
		}
		if (!withdraw && ctx->prefix_count < 256) {
			ctx->prefixes[ctx->prefix_count].key = key;
			ctx->prefixes[ctx->prefix_count].seq_num = seq_num;
			ctx->prefixes[ctx->prefix_count].spf_status = spf_status;
			ctx->prefixes[ctx->prefix_count].prefix_metric = metric;
			ctx->prefixes[ctx->prefix_count].valid = true;
			ctx->prefix_count++;
			ctx->topology_dirty = true;
		}
		break;
...
/*
 * Update topology graph from LSDB (RFC9815).
 * Flow:
 *   1. Check topology_dirty flag (set by LSDB update)
 *   2. Rebuild adjacency graph from valid links
 *   3. Filter by SPF Status: only status=0 (reachable) participates
 *   4. Build node→prefix associations for route calculation
 *   5. Set spf_needed flag if meaningful changes occurred
 *   6. Clear topology_dirty flag
 * 
 * Note: In production this would use an actual graph data structure
 * (adjacency lists, etc.). Here we just mark that graph needs rebuild.
 */
static void bgp_ls_spf_topology_update(struct bgp_ls_spf_ctx *ctx)
{
	if (!ctx->topology_dirty)
		return;

	/* Count reachable elements (SPF Status == 0) */
	uint16_t reachable_nodes = 0, reachable_links = 0;

	for (int i = 0; i < ctx->node_count; i++) {
		if (ctx->nodes[i].valid && ctx->nodes[i].spf_status == 0)
			reachable_nodes++;
	}

	for (int i = 0; i < ctx->link_count; i++) {
		if (ctx->links[i].valid && ctx->links[i].spf_status == 0)
			reachable_links++;
	}

	/* If topology changed meaningfully, trigger SPF */
	if (reachable_nodes > 0 && reachable_links > 0)
		ctx->spf_needed = true;

	ctx->topology_dirty = false;

	if (BGP_DEBUG(linkstate, LINKSTATE))
		zlog_debug("BGPLS-SPF: topology updated (nodes=%u, links=%u)",
			   reachable_nodes, reachable_links);
}

/*
 * Decide whether to run SPF computation (RFC9815).
 * Flow:
 *   1. Check spf_needed flag (set by topology update)
 *   2. Apply debounce: avoid excessive recomputation
 *      - In production: use timer-based debounce (e.g., 500ms)
 *      - Here: simplified immediate trigger
 *   3. Verify prerequisites:
 *      - At least one reachable node
 *      - At least one reachable link
 *   4. Return true to trigger SPF run
 */
static bool bgp_ls_spf_should_run(struct bgp_ls_spf_ctx *ctx)
{
	if (!ctx->spf_needed)
		return false;

	/* Check minimum topology: need nodes and links */
	...
	if (reachable < 2)  /* Need at least 2 nodes for SPF */
		return false;

	/* In production: check debounce timer here */
	return true;
}

/*
 * Run SPF computation and install routes (RFC9815).
 * Flow:
 *   1. Build SPF graph from LSDB:
 *      - Nodes with spf_status=0 as vertices
 *      - Links with spf_status=0 as edges (weighted by igp_metric)
 *   2. Run Dijkstra SPF from local router:
 *      - Compute shortest paths to all reachable nodes
 *      - Calculate path costs using IGP metrics (TLV 1095)
 *   3. For each prefix (TLV 265 + 1155):
 *      - Find originating node via SPF tree
 *      - Compute total cost = SPF_cost + prefix_metric
 *      - Determine next-hop from SPF result
 *   4. Install routes into RIB:
 *      - Call zebra route install APIs
 *      - Set route preference/distance per policy
 *   5. Clear spf_needed flag
 *   6. Log SPF run statistics
 * 
 * Note: This is a placeholder. Production needs:
 *   - Full Dijkstra implementation
 *   - Priority queue for SPF
 *   - Route comparison and update logic
 *   - Integration with existing zebra route APIs
 */
static void bgp_ls_spf_run(struct bgp_ls_spf_ctx *ctx)
{
	if (!ctx->spf_needed)
		return;

	/* Placeholder: actual SPF computation would go here */
	/* Example steps (not implemented):
	 *   1. dijkstra_spf(ctx->nodes, ctx->links, local_router_id)
	 *   2. for each prefix: compute path via SPF tree
	 *   3. bgp_zebra_announce_routes(...) or similar
	 */

	ctx->spf_needed = false;

	if (BGP_DEBUG(linkstate, LINKSTATE))
		zlog_debug("BGPLS-SPF: computation triggered (placeholder)");

	/* TODO: Connect user's existing SPF calculation module here */
}
			     ...
			     struct bgp_nlri *packet, int withdraw)
{
	...
	for (; pnt < lim; pnt += length) {
		...
		p.u.prefix_linkstate.nlri_type = pnt_decode16(&pnt);	// NLRI类型（2字节）
		length = pnt_decode16(&pnt);	// 数据长度（2字节）
...
/*
 * BGPLS-SPF NLRI parser.
 * For now this mirrors the Link-State NLRI loop but leaves a hook to insert
 * SPF-specific handling (e.g., LSDB update) on every NLRI item.
 */
static int bgp_ls_spf_handle_nlri(struct peer *peer, struct attr *attr,
			      const struct prefix *p, afi_t afi, safi_t safi,
			      bool withdraw)
{
	/*
	 * Placeholder: place SPF LSDB update here in the future.
	 * Keep existing behavior so routes still reach the RIB.
	 */

	// 1. 获取 SPF 上下文
	struct bgp_ls_spf_ctx *ctx;
	ctx = bgp_ls_spf_get_ctx(peer);

	// 2. 根据 NLRI 更新 LSDB
    bgp_ls_spf_lsdb_update(ctx, attr, p, withdraw);

    // 3. 通知拓扑更新
    bgp_ls_spf_topology_update(ctx);
...
int bgp_nlri_parse_bgpls_spf(struct peer *peer, struct attr *attr,
		   struct bgp_nlri *packet, int withdraw)
{
	uint8_t *pnt;
	uint8_t *lim;
	...
	/* Process every NLRI in the MP_REACH/MP_UNREACH payload. */
	pnt = packet->nlri;			//当前解析位置
	lim = pnt + packet->length;			//NLRI 结尾位置

	for (; pnt < lim; pnt += length) {
		memset(&p, 0, sizeof(p));

		/* All BGPLS-SPF NLRIs begin with type + length. */
		if (pnt + 4 > lim)
			return BGP_NLRI_PARSE_ERROR_PACKET_OVERFLOW;

		p.u.prefix_linkstate.nlri_type = pnt_decode16(&pnt);		//NLRI 类型（Node / Link / Prefix / SPF扩展等）
		length = pnt_decode16(&pnt);		//NLRI 数据部分长度

		if (pnt + length > lim) {
			flog_err(EC_BGP_LINKSTATE_PACKET,
				 "Link-State NLRI length inconsistent (size %u seen)",
				 length);
			return BGP_NLRI_PARSE_ERROR_PACKET_OVERFLOW;
		}
		...
		if (BGP_DEBUG(linkstate, LINKSTATE))
			zlog_debug("LS SPF Rx %s %s %pFX",
				   withdraw ? "Withdraw" : "Update", afi2str(afi), &p);

		bgp_ls_spf_handle_nlri(peer, attr, &p, afi, safi, withdraw);
	}
