...
static void bgp_process_main_one(struct bgp *bgp, struct bgp_dest *dest,
				 afi_t afi, safi_t safi)
{
	struct bgp_path_info *new_select;	// 获取新的最佳路径
	struct bgp_path_info *old_select;	// 获取旧的最佳路径
...
static void process_subq_early_route(struct bgp_dest *dest)
{
	...
	bgp_process_main_one(table->bgp, dest, table->afi, table->safi);	//执行最佳路径选择
	bgp_dest_unlock_node(dest);
...
static unsigned int process_subq(struct bgp_dest_queue *subq, enum meta_queue_indexes qindex)
{
	...
	STAILQ_REMOVE_HEAD(subq, pq);		// 从队列移除
	STAILQ_NEXT(dest, pq) = NULL; /* complete unlink */

	switch (qindex) {
	case META_QUEUE_EARLY_ROUTE:	//处理早期路由
		process_subq_early_route(dest);
		break;
	case META_QUEUE_OTHER_ROUTE:	//处理普通路由
...
static wq_item_status meta_queue_process(struct work_queue *dummy, void *data)
{
	...
	// 遍历所有子队列（按优先级）
	for (i = 0; i < MQ_SIZE; i++)
		if (process_subq(mq->subq[i], i)) {	// 处理一个节点
			mq->size--;
			break;
		}
			  ...
			  int (*mq_add_func)(struct meta_queue *mq, void *data))
{
	...
	if (work_queue_empty(bgp->process_queue))
		work_queue_add(bgp->process_queue, bgp->mq);	//激活工作队列

	return mq_add_func(bgp->mq, data);	//// 添加到子队列
}
				 ...
				 safi_t safi, bool early_process)
{
	...
	//将刚更新的路由移到链表头部，确保它会在最佳路径选择时被优先考虑。
	...
	//加入相应的处理队列
	if (early_process)
		early_route_process(bgp, dest);		// 高优先级队列
	else
		other_route_process(bgp, dest);		// 普通优先级队列
		...
		struct bgp_route_evpn *evpn)
{
	...
	has_valid_label = (num_labels && bgp_is_valid_label(&label[0]));
	if (num_labels &&
	    ((afi == AFI_L2VPN && safi == SAFI_EVPN) || has_valid_label)) {
		bgp_labels.num_labels = num_labels;
	...
	if (((afi == AFI_IP || afi == AFI_IP6) &&
		...
		if (bgp_find_or_add_nexthop(bgp, bgp, nh_afi, safi, new, NULL,
				    connected, bgp_nht_param_prefix, NULL) ||
		    CHECK_FLAG(peer->flags, PEER_FLAG_IS_RFAPI_HD)) {
			if (accept_own)
