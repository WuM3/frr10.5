...
# 检测 zebra 路径
ZEBRA_PATH=""
if [ -f "$SCRIPT_DIR/../zebra/.libs/zebra" ]; then
...
# 创建测试接口（这是要通告的链路）
ip netns exec router1 ip link add eth0 type dummy
ip netns exec router1 ip addr add 192.168.1.1/24 dev eth0
ip netns exec router1 ip link set eth0 up

echo -e "${GREEN}✓ 网络环境创建完成${NC}"
echo "  Router1: 10.0.0.1 (测试接口 eth0: 192.168.1.1)"
echo "  Router2: 10.0.0.2"
echo ""
...
router bgp 65001
 ...
 ! neighbor 10.0.0.2 update-source veth-r1
 !
 ...
 linkstate monitor interface eth0
exit
!
line vty
!
...
router bgp 65001
 ...
 ! neighbor 10.0.0.1 update-source veth-r2
 !
 address-family link-state link-state
...
# 在router1命名空间中启动tcpdump抓包
ip netns exec router1 tcpdump -i veth-r1 -w /tmp/bgpls_test.pcap -n tcp port 179 >/dev/null 2>&1 &
TCPDUMP_PID=$!
...
grep -i "linkstate_poll_timer\|linkstate_add\|encode_link_attributes" /tmp/frr_router1.log | tail -5
...
# ============================================================================
# 步骤6: 等待Link-State轮询发生
# ============================================================================

echo -e "${BLUE}步骤6: 等待Link-State自动轮询${NC}"

echo "说明: 轮询间隔为30秒，已在初始化时触发过一次"
echo "      现在等待第二次轮询并观察BGP UPDATE消息..."
echo ""

# 等待足够长的时间让第二次轮询发生（30秒轮询间隔 + 缓冲）
echo "等待中（最多35秒）..."
for i in {35..1}; do
    echo -ne "\r  剩余 $i 秒...     "
    sleep 1
...
# 通过日志判断轮询是否发生了
if grep -q "bgp_linkstate_poll_timer.*Polling" /tmp/frr_router1.log; then
    echo -e "${GREEN}✓ Link-State轮询已执行${NC}"
    LINKSTATE_WORKS="yes"
else
    echo -e "${YELLOW}! 未检测到Link-State轮询${NC}"
    LINKSTATE_WORKS="no"
fi
